...
export function useGeminiLiveHook(videoElementId) {
  ...
  const sendTextMessage = useCallback((msg) => {
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      ...
      // 즉시 로컬 conversation 상태에 추가 (사용자 경험 향상)
      const userMsg = { 
        role: 'user', 
        text: msg, 
      ...
      setConversation(prev => {
        ...
        if (contextRef.current.sessionId) {
          supabase.from('sessions').update({ transcript: next }).eq('id', contextRef.current.sessionId);
        }
        return next;
      });
      ...
      ws.onopen = () => {
        ...
        const setupMessage = {
          setup: {
            ...
            systemInstruction: {
              parts: [{ text: `${project.ai_prompt}\n\n### MISSION MAPPING & DATA STORAGE\n도구 호출 시 아래 mission_id 별칭을 정확히 사용하고, 데이터는 result_data의 'value' 키로 기록하세요.\n${missionDesc}` }]
            },
            realtimeInputConfig: {
  ...
  return { status, isSpeaking, logs, setLogs, startSession, stopSession, sendTextMessage, transcript, conversation, setConversation, currentStageIndex, completedMissions }
}
