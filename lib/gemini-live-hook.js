...
export function useGeminiLiveHook(videoElementId) {
  ...
  const startSession = useCallback(async (project) => {
    try {
      ...
      const apiKey = process.env.NEXT_PUBLIC_GOOGLE_GENERATIVE_AI_API_KEY || 'AIzaSyBGAjVEcTw0qtJaCwsEz-73k7u-l2ZppoE';
      const modelIdentifier = 'models/gemini-2.0-flash-exp';
      ...
      addLog(`[시스템] Gemini 2.0 Flash Live (v1alpha) 접속 시도 중...`)
              ...
              functionDeclarations: [
                {
                  ...
                  description: "사용자와의 대화 중 미션의 결과를 저장하거나 수집된 정보를 기록합니다. 각 단계(Stage) 내의 미션을 완수할 때마다 호출하세요.",
                  parameters: {
                    type: "object",
                    properties: {
                      stage_id: { type: "string", description: "현재 수행 중인 단계의 아이디" },
                      mission_id: { type: "string", description: "설계된 미션의 고유 아이디 또는 제목" },
                      ...
                      result_data: { type: "object", description: "수집된 정보 필드나 검증 상세 내용" }
                    },
          ...
          if (sc.model_turn || sc.modelTurn) {
            ...
            if (mt.parts) {
              mt.parts.forEach((p, idx) => {
                console.log(`[GEMINI WS] Part ${idx}:`, p); // 개별 파트 상세 구조 확인
                if (p.text) addLog(`[AI TEXT PART] ${p.text}`)
              })
            }
          }
